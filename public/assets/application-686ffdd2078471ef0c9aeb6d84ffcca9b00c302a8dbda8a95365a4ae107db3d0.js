...
(function() {
  var addMarker, clearData, createActionButtons, createTableLine, createWindLayerInto, current, exports, highlightTableLine, noResultTableLine, putParticle, searchBusStops, setCurrentPosition, setLatLng, setResult;
  ...
  $(function() {
    var $form, fire_status, latitude, longitude, map, marker;
    if ($('#bus_stop_list_map').is(':visible')) {
      ...
      $form.on('ajax:success', function(e, result, status, xhr) {
        setResult(result);
      });
    ...
    if ($('#bus_stop_show_map').is(':visible')) {
      ...
      marker = L.marker([latitude, longitude], {
        icon: selectIcon(fire_status)
      }).addTo(map);
      map.setView(marker.getLatLng(), map.getMaxZoom());
    }
    $('#pager').on('ajax:success', function(e, result, status, xhr) {
      var $page;
      $('#pager').html(result.paginator);
      $('#pager').find('a').each(function(index, dom) {
        var href;
        href = dom.href;
        if (exports.isBlank(href) && href.indexOf("page=") === -1) {
          if (href.indexOf("?") === -1) {
  ...
  exports.create_leaflet_map = function(map_id) {
    var map;
    map = L.map(map_id);
  ...
  exports.initMap = function(map, lat, lng, zoom) {
    if (exports.isBlank(lat) || exports.isBlank(lng)) {
      lat = 20;
      lng = 101;
    }
    zoom = exports.isBlank(zoom) ? 10 : zoom;
  ...
  exports.cantGetPosition = function() {
    return alert("現在地を取得できませんでした");
  };

  exports.highlightTableLine = function($dom, $predom) {
    if ($predom) {
      $predom.removeClass("success");
    }
    if ($dom) {
      return $dom.addClass("success");
    }
  };

  exports.scrollToDom = function($dom, scroll_area_id) {
    var list, offset;
  ...
  exports.newHeatmapLayerInto = function() {
    var cfg;
  ...
  exports.createHeatmapLayerInto = function(map) {
    var cfg, heatmapLayer;
  ...
  createWindLayerInto = function(map) {
    var plotLayer, svgLayer;
        ...
        $.getJSON(windData, function(wind) {
          $.getJSON(fireData, function(data) {
            data.forEach(function(fire) {
              var interval, pos, rect, size, v, windDir;
              pos = map.latLngToLayerPoint(new L.LatLng(fire.lat, fire.lng));
              interval = fire.value > 400 ? 10 : fire.value > 300 ? 4000 - (fire.value * 10) : 1000;
              size = 10;
              rect = {
                left: pos.x - size,
                top: pos.y - size,
  ...
  putParticle = function(svgLayer, x, y, vx, vy) {
    var circle;
    circle = svgLayer.append('circle').attr('cx', x).attr('cy', y).attr('r', 2).attr('fill', '#ffb').attr("stroke", "#fb9").attr('class', 'particle');
    circle.transition().duration(1000).ease(d3.easeLinear).attr('transform', 'translate(' + vx + ',' + vy + ')').on('end', function() {
  ...
  exports.selectIcon = function(fire_status) {
    var icon, icons;
    icons = {
      fire_found: fireIcon_found,
      fire_got_under: fireIcon_gone
    };
    if ((fire_status == null) || fire_status === "") {
      return icons['fire_found'];
    }
    return icon = icons[fire_status];
  };

  addMarker = function(latitude, longitude, name, status) {
    return L.marker([latitude, longitude], {
      icon: selectIcon(status)
    }).bindPopup(name).addTo(current.drawLayer);
  };

  createTableLine = function(val, marker) {
    var tr;
    tr = $('<tr>').attr({
      id: "stop_" + val.id
    });
  ...
  createActionButtons = function(id) {
    var $show, path;
    path = "bus_stops/";
...
(function() {
  ...
  $(function() {
    ...
    if ($('#back_btn').is(':visible')) {
      ...
      return $('#back_btn').on('click', function(e) {
        window.history.back();
      });
    }
  });
...
(function() {
  ...
  exports.loadTimeDimension = function(map) {
    var player, testFIRELayer, timeDimension, timeDimensionControl, timeDimensionControlOptions;
    console.log('LOAD!!');
    ...
    return testFIRELayer.addTo(map);
  };
  ...
  L.TimeDimension.Layer.FIREHeatMap = L.TimeDimension.Layer.extend({
    ...
    onAdd: function(map) {
      ...
      if (this._timeDimension) {
        return this._getDataForTime(this._timeDimension.getCurrentTime());
      }
    },
    _onNewTimeLoading: function(ev) {
      return this._getDataForTime(ev.time);
    },
    isReady: function(time) {
    ...
    _getDataForTime: function(time) {
      ...
      return $.getJSON('/data/fire.json', (function(data) {
  ...
  L.timeDimension.layer.fireHeatMap = function(options) {
